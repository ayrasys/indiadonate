
<link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">
<link href="//cdn.datatables.net/1.10.19/css/jquery.dataTables.min.css" rel="stylesheet" id="bootstrap-css">

<script src="//netdna.bootstrapcdn.com/bootstrap/3.1.0/js/bootstrap.min.js"></script>
<script src="//code.jquery.com/jquery-1.11.1.min.js"></script>
<!-- include summernote css/js -->

<script src="//cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
<script src="//cdn.datatables.net/1.10.19/js/jquery.dataTables.min.js"></script>
<script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>
<link href="https://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.12/summernote-bs4.css" rel="stylesheet">
   <script src="https://cdnjs.cloudflare.com/ajax/libs/summernote/0.8.12/summernote-bs4.js"></script>

<div class="page-header">
  <center>
        <h1>Project Narative List<span class="pull-right label label-default">:)</span></h1>
  </center>
    </div>
    <div class="row">
    	<div class="col-md-12">
        <nav class="navbar navbar">

</nav>

              <!-- ajcode for dataTables -->
        <table id="exampleNarative" class="table table-striped table-bordered" style="width:100%">
        <thead>
            <tr>
                <th>S#</th>
                <th>Date</th>
                <th>Project</th>
                <th>Details</th>
                <th>Attachment</th>
                <th>Status</th>
                <th>Action</th>
            </tr>
        </thead>
        <tbody>
          <?php



          global $wpdb;
          $usid = get_current_user_id();

          if( $_GET["ng"] || $_GET["nid"] ) {
              $ngVal=$_GET["ng"];
              $nid=$_GET["nid"];
              $table_name='wp_ngo_narative';
              if($ngVal=='approved'){
                $wpdb->update( $table_name, array( 'status' => 1),array('id'=>$nid));

              }
              if($ngVal=='declined'){
                  $wpdb->update( $table_name, array( 'status' => 2),array('id'=>$nid));
              }


          }



          $table_name = "wp_ngo_narative";
          $result = $wpdb->get_results( " SELECT * FROM $table_name ");
          $i=0;
          foreach( $result as $printRow ){
            $ida=$printRow->id;

            $i++;
            $flink=wp_get_attachment_link( $printRow->project_attchment, $unfiltered );
            if($printRow->status==1){
              $st='Approved';
            }
            if($printRow->status==2){
              $st='Decline';
            }
            if($printRow->status==0){
              $st='Waiting for Approved';
            }
            $ng_approved = admin_url('/admin.php?page=ngo_project_narrative&ng=approved&nid=');
            $ng_decline = admin_url('/admin.php?page=ngo_project_narrative&ng=declined&nid=');


            ?>
            <tr>
                <td><?php echo $i?></td>
                <td><?php echo $printRow->created_at?></td>
                <td><?php echo $printRow->project_name?></td>
                <td><?php echo $printRow->project_detail?></td>
                <td><?php echo $flink?></td>
                <td><?php echo $st?></td>
                <td>
                  <a href="<?php echo $ng_approved.$ida ?> ">Approved</a> |
                  <a href="<?php echo $ng_decline.$ida ?>">Decline</a> |

                </td>
            </tr>
            <?php

          }

          ?>


        </tbody>

    </table>

              <!-- ajcode for dataTables -->
        </div>

	</div>




  <script type="text/javascript">

  $(document).ready(function() {
    // Setup - add a text input to each footer cell
    $('#exampleNarative thead tr').clone(true).appendTo( '#exampleNarative thead' );
    $('#exampleNarative thead tr:eq(1) th').each( function (i) {
        var title = $(this).text();
        $(this).html( '<input type="text" placeholder="Search '+title+'" />' );

        $( 'input', this ).on( 'keyup change', function () {
            if ( table.column(i).search() !== this.value ) {
                table
                    .column(i)
                    .search( this.value )
                    .draw();
            }
        } );
    } );

    var table = $('#example').DataTable( {
        orderCellsTop: true,
        fixedHeader: true
    } );




  $('#exampleNarative').DataTable({ "order": [[ 0, "desc" ]]});

   $('#summernote').summernote({
        placeholder: 'Enter Project Details',
        tabsize: 2,
        height: 100,
        codemirror: { // codemirror options
      theme: 'monokai'
    }


      });


} );

  </script>
